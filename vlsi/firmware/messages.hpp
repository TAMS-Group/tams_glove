...
static_assert(sizeof(ReceiverMessage) == 8);

struct DeviceType {
  static constexpr uint32_t None = 0;
  static constexpr uint32_t Camera = 1;
  static constexpr uint32_t Radio = 2;
};
...
template <class T> void update_message_checksum(T &message) {
  message.checksum = 0;
  message.checksum = compute_message_checksum(&message, sizeof(T));
}

template <class T> bool verify_message_checksum(T message) {
